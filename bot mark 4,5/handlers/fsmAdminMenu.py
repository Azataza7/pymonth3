...
async def fsm_start(message: types.Message):
    if message.from_user.id in ADMIN:
        ...
        await message.answer(f"Привет бедолага {message.from_user.full_name} "
...
async def load_photo(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    await message.answer("Тип блюда")
...
async def load_value(message: types.Message, state: FSMContext):
    try:
        async with state.proxy() as data:
            data['value'] = f'{int(message.text)} ккал'
        await FSMAdmin.next()
...
async def cancel_registration(message: types.Message, state: FSMContext):
    ...
    if current_state is None:
        ...
        await message.answer("Регистрация блюда отменена!", reply_markup=client_kb.start_markup)
        await state.finish()
...
def register_handlers_fsmmenu(dp: Dispatcher):
    ...
    dp.register_message_handler(load_value, state=FSMAdmin.weight)
